jQuery(document).ready(function ($) {
    ...
    if (pendingBackups.length > 0) {
        ...
        closteCheckStatusInterval = setInterval(checkTasks, 30000);
    }
                ...
                if (response.success) {
                    ...
                    if (response.task.end_date != null) {
                        ...
                        if (response.task.success) {
                            $('.closte_pending_task[data-task-id="' + closte_task_id + '"]').replaceWith('<span>Finished</span>');
                        } else {
    ...
    $(document).on("click", "#closte_clear_cdn_cache", function () {


        $(this).replaceWith("<img src='/wp-admin/images/loading.gif'>");

        var data = {
            'action': 'closte_cdn_invalidation',
        };
